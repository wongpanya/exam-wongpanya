...
### 3.2 องค์ประกอบสำคัญในการตรวจข้อสอบ (Grading Pillars)

การตรวจข้อสอบอัตนัยที่มีความแม่นยำสูงอาศัย 3 องค์ประกอบหลักที่ทำงานร่วมกัน:

```mermaid
graph TD
    A[คำตอบของนักศึกษา] --> D[AI Grading Engine]
    B1[แนวคำตอบ Ground Truth<br/>แกนหลักความถูกต้อง] --> D
    B2[เกณฑ์ Rubric Criteria<br/>มาตรวัดและน้ำหนักคะแนน] --> D
    B3[ประเด็นสำคัญ Key Concepts<br/>จุดเช็คพอยต์สาระสำคัญ] --> D
    D --> E[ผลการตรวจ: คะแนนรายเกณฑ์ + เหตุผล + หลักฐาน]
```

1. **แนวคำตอบ (Ground Truth / Reference Answer)**:
   * **หน้าที่**: ทำหน้าที่เป็น "แกนหลักอ้างอิงความถูกต้อง" (Semantic Anchor) ของข้อสอบ
   * **ประโยชน์**: ช่วยตีกรอบขอบเขตเนื้อหาและทิศทางคำตอบที่อาจารย์ต้องการ ป้องกันไม่ให้ AI หลงประเด็นหรือสร้างข้อสรุปที่ผิดพลาด (Hallucination Mitigation) และเป็นฐานในการเทียบเคียงความสมบูรณ์ของคำตอบ

2. **เกณฑ์การให้คะแนน (Rubric / Multi-Criteria)**:
   * **หน้าที่**: ทำหน้าที่เป็น "มาตรวัดเชิงคุณภาพและปริมาณ" (Standardized Scoring Scale)
   * **ประโยชน์**: แตกข้อสอบข้อใหญ่เป็นเกณฑ์ย่อยที่ชัดเจน (Criterion-Referenceability) ช่วยกำหนดน้ำหนักคะแนนสูงสุดของแต่ละส่วน (`maxScore`) ลดความลำเอียง (Rater Bias) และบังคับให้ AI ต้องให้เหตุผลสนับสนุนทั้งฝั่งคะแนนที่ได้ (`awardedRationale`) และคะแนนที่ถูกตัด (`deductionRationale`)

3. **ประเด็นสำคัญ (Key Concepts / Keywords)**:
   * **หน้าที่**: ทำหน้าที่เป็น "จุดตรวจสอบสาระสำคัญ" (Core Content Checkpoints)
   * **ประโยชน์**: ระบุคำศัพท์เฉพาะทาง, ทฤษฎี, หรือแนวคิดหลักที่จำเป็นต้องปรากฏในคำตอบ ช่วยให้ AI สแกนหาความครอบคลุมของเนื้อหาได้อย่างรวดเร็ว และใช้เป็นจุดจับคู่เพื่อดึงข้อความหลักฐานอ้างอิงตรง (`evidence`) จากคำตอบของนักศึกษา

### 3.3 เกณฑ์การประเมินแบบแยกหัวข้อ (Multi-Criteria Rubric Breakdown)
...
### 3.4 การดึงหลักฐานอ้างอิง (Verbatim Evidence Extraction)
...
### 3.5 Rules Engine (กฎการส่งให้อาจารย์ตรวจสอบ)
